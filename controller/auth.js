...
const sendVerificationCode = async (email) => {
      ...
      <div class="container">
        <div class="header">
          <div class="header-top">
            <div class="logo">
              <img src="cid:logo" alt="CityMovers" style="width: 100%; height: 100%; object-fit: cover;">
            </div>
          </div>
        ...
        <div class="content">
          <div class="greeting">
            Hello! 👋
            <br><br>
            Thank you for signing up with CityMovers. To complete your registration, please use the verification code below.
          </div>
          
          <div class="code-section">
            <div class="code-label">Your Verification Code</div>
            <div class="verification-code">${code}</div>
            <div class="expiry-info">⏱️ Expires in 15 minutes</div>
          </div>
          
          <div class="info-box">
            <p>
              <strong>🛡️ Security Note:</strong> Never share this code with anyone. Our team will never ask for your verification code via email, phone, or any other channel.
            </p>
          </div>
          
          <div class="info-box warning">
            <p>
              <strong>⚠️ Important:</strong> If you didn't request this verification code, please ignore this email or contact our support team immediately.
            </p>
          </div>
          ...
          <div class="cta-section">
            <p>Need help? <a href="mailto:support@citymovers.com">Contact our support team</a></p>
          </div>
        </div>
        
        <div class="footer">
          <p>
            © 2024 CityMovers. All rights reserved.<br>
  ...
  await sendMail(email, "Your CityMovers Verification Code", htmlContent, true, [
    {
      filename: 'logo.png',
      path: './logo/logo.png',
      cid: 'logo'
...
const saveUserLocation = async (req, res) => {
  ...
  try {
    const user = await User.findOneSorted({ createdAt: -1 });
    if (!user) return res.status(404).json({ message: 'User not found' });
...
const getUserByEmail = async (req, res) => {
  ...
  try {
    const user = await User.findOneWithSelect({ email: sanitizedEmail }, '-password -__v');
    if (!user) return res.status(404).json({ message: 'User not found' });
